...
- 主动能力探针（用户每次确认后）：通过当前 Codex 配置发送固定的合成题目，只保留通过数量与耗时。
...
启用本地代理后，请求会被转发到用户在设置中明确填写的上游地址；这替代客户端原本直连该上游的数据路径。运行主动探针时，固定的合成测试题会通过当前 Codex 线路发送，并可能产生模型用量。两项功能均默认不主动运行。
