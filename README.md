...
检测到 Codex 开始处理首个请求后，ModelSentinel 会自动确认是否存在真实响应及响应标识。后续新请求只显示为继续采集，不会覆盖上一轮结果。Codex 当前的本地会话记录不包含服务端独立返回的 `model` 字段，因此被动检测完成后会明确显示“真实响应已确认 / 模型字段未提供”，而不是让用户无限等待；外部代理截获 Responses 对象中的返回模型 ID 后，才会升级为“返回模型已验证”。
