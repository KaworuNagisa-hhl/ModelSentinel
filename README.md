...
检测到 Codex 开始处理首个请求后，ModelSentinel 会自动进入“返回模型验证中”，并在任务结束后确认是否存在真实响应及响应标识。后续新请求只显示为继续采集，不会覆盖上一轮已完成的验证结果。Codex 当前的本地会话记录不包含服务端独立返回的 `model` 字段，因此只能标记“响应已捕获 · 待鉴别”，不会把客户端选择的会话模型误当作服务端模型证明；外部探针提供返回模型 ID 后才会升级为“返回模型已验证”。
