...
如果上游不返回模型字段，或用户不信任该字段，可在设置中手动运行“主动能力探针”。探针使用当前 Codex 配置发送三个合成题目，会消耗少量模型用量；结果只记录通过数量和耗时。行为探针属于概率性风险信号，不是模型身份的密码学证明。

### Token 消耗

- **本地响应代理：0 个额外模型 Token。** 它只转发用户原本就会发送的请求并读取响应元数据，不会另外请求模型。
- **主动能力探针：每次执行 1 个独立 Codex 回合。** 三个合成题目本身约为 100 个输入 Token，但 Codex CLI 还会附带自身的系统指令、工具定义和运行上下文。按当前实现应保守预留约 **10,000–30,000 个输入 Token**，以及约 **100–1,000 个输出/推理 Token**；不同 Codex 版本、模型、推理档位、Provider 和中转计费方式可能使实际用量更高或更低。
